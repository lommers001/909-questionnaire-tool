(function(global, factory) {
    ...
    function CP(source, state = {}) {
        ...
        let className = state['class'],
            ...
            AColor = setElement('input'),
            AColorPrefix = setElement('span'),
            ADropper = setElement('div'),
            SVStarting = 0,
            HStarting = 0,
            AStarting = 0,
            SVDragging = 0,
        ...
        if(window.EyeDropper){
            setChildLast(A, ADropper);
        }
        setChildLast(A, AColorPrefix);
        setChildLast(A, AColor);
        AColor.oninput = function(e){
        ...
        function doApply(isFirst, toParent) {
            ...
            doFit = to => {
                ...
                setStyle(self, 'left', Math.max(400, selfLeft));
            ...
            function doStop(e) {
                theColor = P2RGB(theData);
                if($.visible){
                    source.value = $.color(theColor[0], theColor[1], theColor[2]);
                    source.click();
                    window.editorRef.watchChanges();
                }
                let t = e.target,
...
window.onload = function(e){
    document.querySelectorAll('input[type="color"]').forEach(function(el) {
        ...
        let picker = new CP(el);

        /*picker.on('change', function(r, g, b, a) {
            if(window.isCPLoaded){
                this.source.value = this.color(r, g, b, a);
            }
        });*/
    });

    window.parMedium = new Medium({
		element: document.getElementById("page-par")
	});

    window.headerMedium = new Medium({
		element: document.getElementById("page-header")
	});

    window.setTimeout(function(e){ 
        window.isCPLoaded = true;
    }, 250);
}